C1GADV9	;ADVENTURE.  FLH, DJW/WV ; 29 DEC 82  8:52 AM
	;;1.0;C1GAMES;****;March 1, 2016;
	;FLH ; 29 DEC 82  8:52 AM
	; DJW/WV ;March 1, 2016
HINT	D @("HINT"_J) E  Q:J=5  G HI1
	S X=^C1GADV(71100,"h",J),P=$P(X,"/",2),Q=$P(X,"/",3),Y=0,N=54 D YES E  Q
...
HI1	S HINTLC(J)=0 Q
HINT4	I ^C1GADV(71100.1,G,"GRATE","P")=0 S O="KEYS" D HERE I '$T
	Q
HINT5	I OBJ="BIRD" S O=OBJ D HERE I  I TOTING["/ROD/"
	Q
HINT6	S O="BIRD" D HERE I '$T S O="SNAKE" D HERE
	Q
HINT7	S I=HOLDNG>1 F L=LP,LP1,LP2 Q:'I  I L>0 S I=^C1GADV(71100.1,G,L,"O")="/"
	I I
	K I Q
HINT8	I ^C1GADV(71100.1,G,"EMERA","P")=-1,^C1GADV(71100.1,G,"PLATI","P")=-1
	Q
HINT9	I 1
	Q
...
TRS3	S O="TROLL" I ^C1GADV(71100.1,G,O,"P")=1 S ^("P")=0,P=1 D PSPK S L=117 D MOVE S L=122 D MOVEF S O="TROLL2",L=0 D MOVE,MOVEF S O="CHASM" D JUGGLE Q
	S LNEW=239-LP S:^("P")=0 ^("P")=1 Q:TOTING'["/BEAR/"
	S ^C1GADV(71100.1,G,"CHASM","P")=1,^C1GADV(71100.1,G,"TROLL","P")=2,O="BEAR",L=LNEW,M=162 D DROP,SPK S ^C1GADV(71100.1,G,O,"F")=-1,^("P")=3
	S:^C1GADV(71100.1,G,"SPICE","P")<0 TALLY2=TALLY2+1 S LP2=LNEW,RET="DEAD^C1GADV8" Q
TRB	S L3=$S($E(^C1GADV(71100,LP1,"c")):LP2,1:LP1),LP2=LP1,LP1=LP I L3=LP S M=91 D SPK
	E  S MOV="A" F Z=0:0 S MOV=$O(^C1GADV(71100,LP,MOV)) Q:MOV]"ZZ"  F J=1:1:^(MOV,0) S Y=^(J) I $P(Y,"\",2)="TR2" S L=$P(Y,"\",3),LL(L)="" I L=L3 S LNEW=L3 G TRB1
	E  S L="" F Z=0:0 S L=$O(LL(L)) Q:L=""  I $D(^C1GADV(71100,L,"FORCED",1)),$P(^(1),"\",3)=L3 S LNEW=L3 Q
	E  S M=140 D SPK
TRB1	K L3,LL,MOV Q
TRC	S M=$S(LP<8:57,1:58) D SPK Q
TRL	I DETAIL<3 S M=15 D SPK
	S DETAIL=DETAIL+1,WZDARK=0,^C1GADV(71100.1,G,LP,"A")=0 Q
TRN	Q
